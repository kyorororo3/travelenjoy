...
class TravelReservation extends React.Component {
  ...
  componentDidMount() {
    fetch(`http://localhost:3002/tour/detail/${this.props.location.state.tour_seq}`)
      .then(res => res.json())
      .then(data => {
        ...
        document.getElementsByClassName('data')[3].innerHTML = "₩" + (info.price * this.props.location.state.person);
    ...
    return(
      <div className='travel-reservation-wrapper'>
        ...
        <div className='travel-pay'>
          <button type='button' className='btn-l'>결제하기</button>
        </div>
      </div>
    )
  }
